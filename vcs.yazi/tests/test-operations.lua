return function(t)
	...
	local editor_dir = t.temp_dir()
	local editor = t.path_join(editor_dir, "native-editor.lua")
	local editor_log = t.path_join(editor_dir, "native-editor-input.txt")
	file = assert(io.open(editor, "w"))
	file:write([[local input = assert(io.open(arg[2], "r"))
local template = input:read("*a")
input:close()
local log = assert(io.open(arg[1], "w"))
log:write(template)
log:close()
local message = assert(io.open(arg[2], "w"))
message:write("path commit\n")
message:close()
]])
	file:close()
	git({ "config", "commit.status", "true" })
	git({ "config", "core.editor", "lua " .. t.shell_quote(editor) .. " " .. t.shell_quote(editor_log) })
	git(commands.git_commit({ "space name.txt" }, "paths"))
	local log_file = assert(io.open(editor_log, "r"))
	local editor_template = log_file:read("*a")
	log_file:close()
	t.truthy(editor_template:match("Changes to be committed"), "native Git editor receives the commit status template")
	t.truthy(editor_template:match("space name%.txt"), "native Git editor receives the selected path")
	t.remove_tree(editor_dir)
	...
	local delete_file = t.path_join(dir, "delete file.txt")
	local delete_directory = t.path_join(dir, "delete directory")
	t.make_dir(delete_directory)
	file = assert(io.open(delete_file, "w")); file:write("delete me\n"); file:close()
	file = assert(io.open(t.path_join(delete_directory, "inside.txt"), "w")); file:write("delete directory\n"); file:close()
	git(commands.git_add({ "delete file.txt", "delete directory" }))
	git({ "commit", "-q", "-m", "delete fixtures" })
	git(commands.git_delete({ "delete file.txt", "delete directory" }))
	local deleted_file_handle = io.open(delete_file, "rb")
	if deleted_file_handle then deleted_file_handle:close() end
	local deleted_directory_handle = io.open(delete_directory, "rb")
	if deleted_directory_handle then deleted_directory_handle:close() end
	t.falsy(deleted_file_handle, "[integration] git delete removes a tracked file from the working tree")
	t.falsy(deleted_directory_handle, "[integration] git delete removes a tracked directory from the working tree")
	local delete_status = capture({ "diff", "--cached", "--name-only", "--", "delete file.txt", "delete directory" })
	t.truthy(delete_status:match("delete file%.txt"), "[integration] git delete stages the file deletion")
	t.truthy(delete_status:match("delete directory/inside%.txt"), "[integration] git delete stages recursive directory deletion")
	t.remove_tree(dir)
end
