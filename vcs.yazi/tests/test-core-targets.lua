return function(t)
	...
	local selected_sources = { { path = "/repo/a", is_dir = false }, { path = "/repo/b", is_dir = true } }
	t.deep_eq({ targets.choose_file_sources(selected_sources, { path = "/repo/hover", is_dir = false }) }, { selected_sources, "selected" }, "file operations prefer explicit selection")
	t.deep_eq({ targets.choose_file_sources({}, { path = "/repo/hover", is_dir = false }) }, { { { path = "/repo/hover", is_dir = false } }, "hovered" }, "file operations fall back to hovered item")
	t.deep_eq({ targets.choose_file_sources({}, nil) }, { {}, nil }, "file operations do not fall back to cwd")
	local only, only_reason = targets.single_file_source({ selected_sources[1] })
	t.eq(only, selected_sources[1], "rename accepts exactly one source")
	t.eq(only_reason, nil, "single rename has no error")
	local no_source, no_source_reason = targets.single_file_source({})
	t.eq(no_source, nil, "rename rejects an empty source")
	t.eq(no_source_reason, "no-target", "empty rename source is classified")
	local multiple, multiple_reason = targets.single_file_source(selected_sources)
	t.eq(multiple, nil, "rename rejects multiple sources")
	t.eq(multiple_reason, "multiple", "multiple rename sources are classified")
	t.truthy(targets.validate_basename("-new [*] 日本語.txt"), "rename accepts literal special characters and Unicode")
	t.falsy(targets.validate_basename("."), "rename rejects dot basename")
	t.falsy(targets.validate_basename("../outside"), "rename rejects a path instead of one basename")
	t.eq(targets.rename_path("folder/old.txt", "new name.txt"), "folder/new name.txt", "rename keeps the source parent")

	local move_plan = targets.plan_move("/repo", selected_sources, "/repo/destination", function() return false end, false)
	t.deep_eq(move_plan, {
		paths = { "a", "b" }, destination = "destination",
		targets = { "/repo/destination/a", "/repo/destination/b" },
	}, "move plan preserves root-relative source paths and basenames")
	local root_move = targets.plan_move("/repo", { { path = "/repo/sub/- dash [*] 日本語", is_dir = false } }, "/repo", function() return false end, false)
	t.eq(root_move.destination, ".", "move plan supports repository root destination")
	t.deep_eq(root_move.paths, { "sub/- dash [*] 日本語" }, "move plan preserves unusual source characters")
	local collided, collision_reason = targets.plan_move("/repo", selected_sources, "/repo/destination", function(path) return path == "/repo/destination/a" end, false)
	t.eq(collided, nil, "move plan rejects an existing target")
	t.eq(collision_reason.code, "collision", "target collision is classified")
	local duplicate, duplicate_reason = targets.plan_move("/repo", {
		{ path = "/repo/first/shared.txt", is_dir = false },
		{ path = "/repo/second/shared.txt", is_dir = false },
	}, "/repo/destination", function() return false end, false)
	t.eq(duplicate, nil, "move plan rejects selected sources with the same destination basename")
	t.eq(duplicate_reason.code, "duplicate", "duplicate generated targets are classified")
	local inspect_failed, inspect_reason = targets.plan_move("/repo", { selected_sources[1] }, "/repo/destination", function() return nil, "denied" end, false)
	t.eq(inspect_failed, nil, "move plan rejects an uninspectable target")
	t.eq(inspect_reason.code, "inspect-error", "target inspection failure is classified")
	local inside, inside_reason = targets.plan_move("/repo", { { path = "/repo/folder", is_dir = true } }, "/repo/folder/child", function() return false end, false)
	t.eq(inside, nil, "move plan rejects a destination inside a source directory")
	t.eq(inside_reason.code, "inside-source", "self-descendant move is classified")
	local outside, outside_reason = targets.plan_move("/repo", selected_sources, "/outside", function() return false end, false)
	t.eq(outside, nil, "move plan rejects a destination outside the working tree")
	t.eq(outside_reason.code, "outside", "outside destination is classified")
	...
	t.deep_eq(targets.exclude_untracked({ "tracked-dir", "new-dir" }, { ["tracked-dir"] = "untracked", ["new-dir"] = "untracked" }, { ["tracked-dir"] = true }), { "tracked-dir" }, "tracked directories remain eligible when status was aggregated from an untracked child")
